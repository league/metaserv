...
\cd{printf} by placing the format string immediately after the code
delimiter.\footnote{In a departure from the standard OCaml syntax for
  \cd{printf}, the arguments are comma-delimited.  This way, fewer
...
parameter with a given name.  Since \cd{req} and \cd{puts} are
...
\cd{puts}~\verb+"\n"+ statements).  It automatically appends a
semi-colon or the {\KWfont in} keyword to code blocks, as required
(see, for example, lines 7, 11, and 12 in figure~\ref{fig:trans.ml}.)
%%
Finally, because the translator partially parses the OCaml code
blocks, it is not confused by code delimiters within strings and
comments, nor by other uses of \la{} and \ra{} as operators.
...
time to display the size of the server log file.
...
To stage this, the \cd{stat} and \cd{float\_of\_int} calls could be
lifted into the declaration block, as with the permissions example.
Furthermore, we may also use \cd{lift} and the splice operator to
perform the divisions in advance, even though they are underneath the
{\KWfont match} (which cannot happen until the serve stage):
\lgrindfile{size2.meta}%
...
URI \id{/longcount} to a code block generated with an actual argument
of 99, while \id{/shortcount} refers to code with the actual
...
of web services, organized to comprise a small web site.\footnote{A
  demo is available at \url{http://comsci.liu.edu/metaserv/}} To give
...
for the \cd{CodeHandler} (and two variants):
...
Values of type \cd{map} tell the server which pages are mapped to
which URIs.  Ideally, it would be defined as \cd{page code
  StringMap.t}.  This is not currently possible: the \cd{page code}
needs to be run, but in the current implementation of \MOC, the
\Mrun{} operator cannot be used from within functions.  Passing around
code values themselves has limited utility because they must remain
polymorphic to run them, and OCaml does not yet support rank-2
polymorphism.  (Although recent work by \citet{garrigue99semi} leads
in that direction.)  Our work-around involves mapping the URI instead
to a pair containing a function (defined at the top level) to generate
the page (by running the first stage and then using the \Mrun{}
operator on the generated code) and a ref cell where the resulting
page is cached.
...
features of \MOC.  The relavant fragment of code is shown in
figure~\ref{fig:powerhof.ml}.  Using continuation-passing style, it
