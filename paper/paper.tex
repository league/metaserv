...
\mylst{trans.ml}{An automatic translation of the page in
  listing~\ref{lst:trans.meta}.  Declarations have been lifted, and
...
constructed code follows the line that begins with \cd{{\KWfont let} page}.
...
to the user's browser.  \cd{arg} is then defined to look up an
argument in the query string of the request; its type is \cd{string\to
  string option}.
Since \cd{req} and \cd{puts} are serve-stage arguments, it is
incorrect to invoke them in the \emph{publish} (first) stage, and indeed
the \MOC type system prevents this.
...
semi-colon or the {\KWfont in} keyword to code blocks, as required (see,
for example, the lines containing \cd{statements},
\cd{more\_statements}, and \cd{{\KWfont let} x} in
...
\cd{puts "0755"}.  It is also possible to splice in serve-stage
...
the identifiers \cd{a b c} in listings~\ref{lst:trans.meta}
...
script; see the line marked with {(****)}.  If \cd{num\_of\_string}
...
The \cd{map} type maps a string (the URI) to a pair containing a
function to (re-)generate the page (by running the first stage) and a
ref cell where the page is cached.  No \cd{code} types show up here,
but they are used within the \cd{unit\to page} function.  This
